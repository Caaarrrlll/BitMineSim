...
int main(){
	...
	//Miner Functions
	cout << endl << "Miner Functions" << endl;
	Miner* m = new Miner("Piet");
	//Miner p("Tool");

	//miner
	cout << "Miner m name: " << m->getName() << endl;
	//cout << "Miner p name: " << p.getName() << endl;
	...
	m->createBlock(pool, &b1, 10.50);

	cout << "After block created" << endl;

	Block b2(*(m->getBlock()));
	b2.printBlockSummary();

	//m.deleteBlock();
	cout << "Block Confirm" << endl;
	m->confirmBlock();

	cout << "Wallet" << endl;
	cout << m->getWallet()->getBalance() << endl;

	cout << "Mine" << endl;
	cout << m->mine(157) << endl;
